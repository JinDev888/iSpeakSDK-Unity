...
async function sendContract(method, abi, contract, args, value, gasLimit, gasPrice) {
  const from = (await web3.eth.getAccounts())[0];
  let arg_list = JSON.parse(args);
  if (method == "safeTransferFrom") // the Last parameter must be byte array in ERCF1155's safeTransferFrom
  {
    let utf8Encode = new TextEncoder();
    arg_list[4] = utf8Encode.encode(arg_list[4]);
  }
  new web3.eth.Contract(JSON.parse(abi), contract).methods[method](...arg_list)
      .send({
        from,
        value,
        gas: gasLimit ? gasLimit : undefined,
